...
from tiztoken import BasicTokenizer, RegexTokenizer, GPT4Tokenizer
...
def unpack(text):
    ...
    # and we don't want to print the entire content of the file
    if text.startswith("FILE:"):
        dirname = os.path.dirname(os.path.abspath(__file__))
        # we skip the first 5 characters, which are "FILE:"
        test_text_file = os.path.join(dirname, text[5:])
        content = open(test_text_file, "r", encoding="utf-8").read()
        return content
    return text
...
# test that our tokenizer matches the official GPT4 tokenizer
@pytest.mark.parametrize("text", test_strings)
